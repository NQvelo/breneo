...
export function AppSidebar({ collapsed, toggleSidebar }: AppSidebarProps) {
  const navItems = [
    ...
    { icon: Settings, label: 'Courses', href: '/courses' }
  ];
    ...
    <div className={cn(
      ...
      <div className="p-4 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center">
          ...
          {collapsed && (
            <img src="/lovable-uploads/6bee4aa6-3a7f-4806-98bd-dc73a1955812.png" alt="Breneo Logo" className="h-10 w-10" style={{ objectFit: 'cover', objectPosition: 'left' }} />
          )}
        </div>
        <Button 
          variant="ghost" 
        ...
        <nav className="space-y-2 px-2">
          {navItems.map((item, index) => (
            <Link
              ...
              to={item.href}
              className={cn(
                "flex items-center space-x-3 px-3 py-3 rounded-md transition-all",
                "hover:bg-white/10",
