  ...
  return <div className={cn("h-screen fixed top-0 left-0 z-40 bg-breneo-navy text-white transition-all duration-300 flex flex-col", collapsed ? "w-20" : "w-64")}>
      ...
      <div className="flex-1 overflow-y-auto py-6 bg-white">
        <nav className="space-y-2 px-2">
          {navItems.map((item, index) => <Link key={index} to={item.href} className={cn("flex items-center space-x-3 px-3 py-3 rounded-md transition-all", "hover:bg-white/10", window.location.pathname === item.href ? "bg-white/20" : "")}>
              <item.icon size={20} />
